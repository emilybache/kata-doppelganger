...
def test_divide_should_not_raise_any_error_when_authorized_with_spy():
    spy = Mock()
    spy.authorize = Mock(return_value=True)
    calculator = SafeCalculator(spy)
    assert calculator.add(10, 2) == 12
    spy.authorize.assert_called_once()


def test_divide_should_raise_error_when_not_authorized_with_spy():
    spy = Mock()
